...
struct parameters *copy_parameters(struct parameters *cp)
{
    struct parameters *cpcopy = calloc(1,sizeof(struct parameters));
    cpcopy->gfpower = cp->gfpower;
    cpcopy->pktsize =  cp->pktsize;
    cpcopy->repfreq = cp->repfreq;
    cpcopy->seed = cp->seed;
    return cpcopy;
}

unsigned char *recover_packet(struct decoder *dc,int sourceid)
{
    int pktsize = dc->cp->pktsize;
    unsigned char *pktstr = calloc(pktsize, sizeof(unsigned char));
    memcpy(pktstr, dc->recovered[sourceid], pktsize*sizeof(GF_ELEMENT));
    return pktstr;
}
